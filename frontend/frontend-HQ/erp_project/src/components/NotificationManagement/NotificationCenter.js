...
export default function NotificationCenter() {
  ...
  const fetchNotifications = async (page = 0, isReset = false) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/notifications/headquarters?page=${pageNumber}&size=20`);
      ...
      console.log(`본사 알림 목록 조회 완료 - 페이지: ${page}, 받은 데이터:`, data);
    ...
    return (
      <div className={styles.container}>
        <div className={styles.header}>
          <h1 className={styles.title}>본사 알림 센터</h1>
        </div>
      ...
      <div className={styles.header}>
        ...
        <div className={styles.headerActions}>
          <div className={styles.statusInfo}>
            <span className={`${styles.connectionStatus} ${websocketConnected ? styles.connected : styles.disconnected}`}>
              {websocketConnected ? '🟢 실시간 연결됨' : '🔴 연결 끊김'}
            </span>
